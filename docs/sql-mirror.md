...
The mysqli path works with either mysqli error-reporting mode, so an existing
...
- Restore: the files are gone but the mirror survives. Register a fresh store
  under the same collection name and pull.
- Seeding: rows loaded into the mirror table by external tooling become storh
  records. Rows must carry valid UUIDv7 ids and an object in the `data`
  column; typed columns are ignored on pull because `data` is the whole
  record.
...
Pulled writes go through the normal store machinery, so schema validation and
