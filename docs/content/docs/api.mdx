...
The methods listed here are the documented 0.0.2 API. Some classes expose
...
Requires `ext-pdo` with the `sqlite` or `mysql` driver.

- `new SqlMirror(PDO $pdo, string $prefix = 'storh_', ?string $driver = null)`
