...
async function getLatestVersion(): Promise<string | null> {
  try {
    const res = await fetch(
      "https://api.github.com/repos/inline0/storh/releases/latest",
  ...
  return (
    <>
      ...
      <p className="text-left max-w-xl leading-normal text-fd-muted-foreground sm:text-lg sm:leading-normal text-balance mt-4">
        JSONC documents, append-only segmented logs, and log-backed queues for
        applications that want durable local storage without a database
        server. Zero dependencies.
      </p>
      <div className="mt-8 w-full">
        <CodeBlock
          lang="bash"
          code="composer require storh/storh"
          className="!my-0"
        />
        <div className="flex items-center gap-3 mt-4">
          <Button href="/docs">Get Started</Button>
          <a
            href="https://github.com/inline0/storh/releases"
            target="_blank"
