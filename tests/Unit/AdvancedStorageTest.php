...
final class AdvancedStorageTest extends TestCase
{
    ...
    public function test_doc_store_query_builder_indexes_schema_cache_bulk_and_maintenance(): void
    {
        ...
        $this->assertSame(
            'home',
            $store->query()->where('publishedAt')->gte(10)->orderBy('publishedAt')->first()?->data()['slug'] ?? null
        );
        ...
        $this->assertSame(
            'notes',
            $store->query()->where('publishedAt')->gte(10)->orderBy('publishedAt', 'desc')->first()?->data()['slug'] ?? null
        );
        ...
        $this->assertSame('stream-a', $streamed->query()->first()?->data()['slug'] ?? null);
        $this->assertCount(1, $streamed->query()->limit(1)->get());
        $this->assertSame(2, $streamed->query()->where('kind')->eq('page')->count());
        $this->assertSame('stream-a', $streamed->query()->where('kind')->eq('page')->first()?->data()['slug'] ?? null);
        $this->assertSame(
            array( 'stream-a', 'stream-b' ),
            array_map(static fn(StorageRecord $record): string => $record->data()['slug'], $streamed->query()->where('kind')->eq('page')->get())
        );
        $this->assertSame(
            array( 'stream-a', 'stream-b' ),
